...
- **Apple Health: you choose what to allow.** When you tap **Connect Apple Health**, Apple's permission sheet lists every health data type the app can use — the table in section 3.1 — whichever conditions you track, and for each type you allow, the app imports the last 400 days of it (records of the symptom types it asks for in full). If you turn off **Import everything from Health** in Settings, the sheet lists only the types the conditions you track use, and the app then reads no other type, even one you allowed on an earlier sheet. It never asks for reproductive or sexual health data, your date of birth, biological sex or blood type. Section 3.1 lists everything it can ask for and what it never asks for.
...
**You are not obliged to grant any of it.** Apple's sheet has an individual toggle for every category, and the app works with whatever subset you allow. If you only want blood pressure to come through, turn on only blood pressure — or turn off **Import everything from Health** first, so that the sheet lists only what your conditions use. You can also skip Apple Health entirely and type readings in.
...
- Guideline 5.1.1 also asks an app to request only data relevant to what it does. By default Connect Apple Health asks for the types in the table in section 3.1, each of which a condition programme or one of the app's screens uses, and for nothing else in Apple Health; with **Import everything from Health** off it asks for, and reads, only the types your conditions use. The app's Health permission text says the same.
...
- **Handoff** passes which tab of the app you are on between your own nearby devices (section 5).
...
- **Apple Health.** **Connect Apple Health** now asks for every type in the table in section 3.1 — every type the app can use — whichever conditions you track, and imports the last 400 days of each type you allow (records of the symptom types it asks for in full). **Import everything from Health** in Settings is now one switch for the whole request, on by default; turning it off limits the request, and what the app reads, to the types your conditions use. Before, it added only General health's extra types to a request that otherwise covered your conditions alone. Section 3.1 says which types in the table no condition programme uses and which of the app's screens use them, and section 14 that **Delete everything this app stored** turns the switch back on.
- **Handoff** now passes only the tab you are on, not the Learn topic you are reading (section 5).
- **Deletion.** Section 14 lists which Assistant settings **Delete everything this app stored** keeps. What it deletes and keeps did not otherwise change.
...
- **Apple Health.** It says that **Connect Apple Health** asks only for the types the conditions you track use (General health's extra types only with **Import everything from Health** on) and imports the last 400 days of each of those types you allow (records of the symptom types it asks for in full). Earlier versions described a separate "Import all Health history" action, which no longer exists. The table in section 3.1 now also lists breathing disturbances during sleep and Apple's hypertension and sleep apnea notifications, and it names the types in it that no condition programme uses in this version, so the sheet never lists them. It also says what the app keeps about the device that measured an imported reading, and that nothing is read in the background.
