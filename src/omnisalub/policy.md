...
- **Apple Health: you choose what to allow.** When you tap **Connect Apple Health**, Apple's permission sheet lists every health data type any of the app's condition programmes can use — not only the ones for the conditions you switched on — and for each type you allow, the app imports the last 400 days of it (your symptom records in full). It never asks for reproductive or sexual health data, your date of birth, biological sex or blood type. Section 3.1 lists exactly what it asks for and what it does not.
...
**What the permission sheet asks for — corrected on 23 September 2026.** Earlier versions of this policy said the sheet is built from the conditions you have switched on — for example, eight types if you track blood pressure alone. That is not what the app does. When you tap Connect Apple Health, Apple's sheet lists **every type in the table below** — the complete set that any of the app's condition programmes can use — whichever conditions you chose. For each type you allow, the app then **imports your history** from Apple Health into its database on your device — the last 400 days of readings, sleep, workouts and Watch notifications, and all of your symptom records — and afterwards refreshes the types your active conditions use (for General health, including its extra types if you turned on **Import everything from Health**; that switch no longer changes what the permission sheet asks for). Each later tap does the same again for anything new. Apple's sheet appears only for types you have not already answered.

The table below is therefore what the sheet asks for, not a ceiling:
...
- Guideline 5.1.1(i) also asks an app to request only the data it needs. Every type in the table in section 3.1 is one that at least one of the app's condition programmes reads; as section 3.1 explains, Connect Apple Health currently asks for all of them rather than only those your conditions use.
...
| A "where you left off" note — which tab you were on, the identifier of the Learn topic you last opened, when, and your appearance choice | Apple's iCloud key-value storage on this device | Not in this version (section 5) |
...
Separately from settings sync, and whether or not you have turned it on, the app writes a small note through Apple's iCloud key-value storage: which tab you were on, the identifier of the Learn topic you last opened (for example a topic about PSA test numbers), the time, and your appearance choice. It never contains a reading. It exists so that another device could offer to continue where you left off. **This version of the app is not signed with Apple's iCloud key-value storage capability, so the note is not uploaded to iCloud and does not leave the device.** One consequence is that the Continue row on Apple TV has nothing to show in this version. **Delete everything this app stored** clears the note.
...
- is **kept** when you use **Delete everything this app stored**, which adds an entry recording the erase. Deleting the app removes it.
...
- **Settings → Your data → Delete everything this app stored** erases your readings, symptoms and alerts immediately, and resets the conditions you chose, your visit date, visit questions and reminder switch, your food-and-weight answer, and your theme, guideline set, app lock and last tab — on this device, and in your iCloud copy of those settings if sync is on (the iCloud record is reset to defaults rather than removed). It also cancels pending visit reminders, clears the "where you left off" note, the widget's file and the bookmarks of how far the Apple Health import got, and returns you to the first-run screens.
- **What Delete everything keeps:** anything in Apple Health, and the Apple Health permissions you granted (change those in your device's Settings); the downloaded Ask model and your Assistant settings; the activity log (section 12); your choice about iCloud settings sync; the **Show today's reading on the Home Screen** switch; and your Pro purchase, which is Apple's record.
...
- **Apple Health.** It says that **Connect Apple Health** asks for every type in the table in section 3.1, whichever conditions you track, and imports the last 400 days of each type you allow (symptom records in full). Earlier versions said the request was built from your active conditions (eight types for blood pressure alone) and described a separate "Import all Health history" action as the only way to ask for the whole table; neither describes the current app. The table now also lists breathing disturbances during sleep and Apple's hypertension and sleep apnea notifications, which the app already asked for. It also says what the app keeps about the device that measured an imported reading, and that nothing is read in the background.
...
- **iCloud and Handoff.** It says settings sync now carries six items, adding the tab you last had open; describes the "where you left off" note, which in this version stays on the device; and describes Handoff.
...
- **Deletion.** It describes **Delete everything this app stored**, which now also resets your conditions, visit date and preferences, and lists what it keeps, including the activity log.
