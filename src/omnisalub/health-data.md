...
| **Delete** | Have your consumer health data deleted | Because we hold nothing, there is nothing for us to delete. On your device: **Settings → Your data → Delete everything this app stored** erases your readings, symptoms and alerts and resets your conditions, visit date and preferences; among other things it keeps the activity log (which holds event types, never values), the downloaded Ask model and anything in Apple Health (the full list is in section 14 of the OmniSalub Privacy Policy). Deleting the app removes everything the app stored on that device. Data already in Apple Health is managed by you in the Health app |
...
- Section 8 now lists what the app handles on your device, and says that **Connect Apple Health** asks for every type any of the app's condition programmes can use and imports the last 400 days of each type you allow (symptom records in full).
