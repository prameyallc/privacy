...
- **iCloud Sync is off until you turn it on.** When it is on, it carries a small set of app settings and some skin-care topics you have opened to your own iCloud account — never your journal, photographs or habit logs. Separately, **Handoff** can tell your other Apple devices which tab and topic you are on (see "iCloud, Handoff and your other devices").
...
| Your goals list, reminder on/off and hour, appearance, landing tab, first-run acknowledgement, the iCloud Sync switch, whether citations start expanded, and an internal reference to the journal entry you saved last | On your device, in app preferences | Only inside your device backup — and, **if** you turn on iCloud Sync, the settings listed under "iCloud, Handoff and your other devices" |
...
| The topic held for continuing, the next suggested topic, and the list of topics opened from the widget or continued from another device (see "iCloud Sync") | In **your own iCloud key-value storage**, only while iCloud Sync is on | Yes — to your iCloud, not to us |
...
|---|---|---|
| MiniCPM5 2B ("Compact") | `openbmb/MiniCPM5-2B-MLX` (`8a9ad753`) | about 1.4 GB |
| Gemma 4 E2B ("Standard") | `mlx-community/gemma-4-E2B-it-qat-4bit` (`42f62737`) | about 4.4 GB |
| Gemma 4 E4B ("Large") | `mlx-community/gemma-4-E4B-it-qat-4bit` (`0f35c6f6`) | about 6.8 GB |
...
**Off by default.** The switch is **Settings → iCloud Sync (app preferences only)**. While it is off, OmniDerm writes nothing to iCloud. While it is on, it uses two parts of **your own iCloud account**:

**1. A preference record in your private CloudKit database.** When you switch sync on, and each time the app launches while it is on, OmniDerm reads this record and applies it. It writes the record only when you tap **Sync Now**. The record can hold only these settings, each checked in code before it is written so free text cannot ride along: appearance mode, whether reminders are on, the reminder hour, which tab the app opens on, whether you have acknowledged the app's disclosure, whether citations are expanded by default (each of those limited to a fixed set of values), and the identifier of an on-device model from an older version of the app (limited to a short run of letters, digits and a few punctuation marks, with no spaces). This version never records your Ask model choice in that field, so that choice is not synced; the field carries a value only if an older version put one there.
...
**To stop it:** switch **iCloud Sync** off. That stops every write, and removes OmniDerm's key-value entries from iCloud straight away (on every device, because the store is shared); Clear All Local Data does the same. **Neither deletes the preference record in CloudKit**: to remove it, delete OmniDerm's iCloud data in **iOS Settings → your name → iCloud → Manage Account Storage**. Deleting the app does not by itself remove anything from iCloud.

With sync off, the widget shows the first suggested topic and Apple TV has nothing to continue. The Apple Watch's **Read this topic** card does not depend on iCloud Sync: it goes over the connection between your iPhone and Watch.

### Handoff

While OmniDerm is open, iOS advertises it for Handoff so your other devices signed in to the same Apple Account can offer to continue. The Handoff record names the tab you are on and, when that tab is Learn, the identifier of the topic held for continuing (described under "iCloud Sync" above). **This happens whether or not iCloud Sync is on.** Handoff is Apple's service; the record is not public and is not indexed for search. You can turn Handoff off in **iOS Settings → General → AirPlay & Continuity** (on a Mac, in System Settings → General → AirDrop & Handoff).
...
- On iPhone, iPad and Apple Vision Pro, OmniDerm declares `NSFileProtectionComplete` (`com.apple.developer.default-data-protection`), so journal photographs, notes, body areas and habit logs are unreadable while your device is locked. An export you prepare is written with the next level down, readable after first unlock, so the share sheet can finish after the screen locks, and is cleared afterwards. On a Mac, the files are protected by macOS and by FileVault if you use it.
...
- **Delete your habit logs, journal entries, photographs, goals and reminder settings:** open **Settings → Export & Data Management → Clear All Local Data**. It deletes your journal entries and photographs, habit logs, goals, reminder setting and pending reminders, the downloaded Ask model and your answer to its download, switches iCloud Sync off and removes OmniDerm's iCloud key-value entries. It does not remove the CloudKit preference record (see "iCloud Sync") or your purchases. If a later file removal fails after the store is emptied, the app says so rather than claiming a complete delete.
...
- **Delete synced data:** switch iCloud Sync off (removes the key-value entries), then remove the app's iCloud data in iOS Settings as described above (removes the preference record).
...
- Information about your skin and your health is **special category data** under Article 9 of the GDPR and the UK GDPR. Your journal, photographs and habit logs are processed **only on your own device** (and in any device backup you make), at your initiative, under your explicit choice to use the feature. If you turn on iCloud Sync, some topics you have opened are stored in your own iCloud account by Apple; whether or not it is on, Handoff passes the tab and topic you are on to your own devices through Apple. Prameya does not receive any of it and does not process it.
...
- **iCloud.** It says iCloud Sync also stores, in your own iCloud key-value storage, the topic held for continuing, the next suggested topic and a list of topics opened from the widget or continued from another device, so your Watch, Apple TV and widget can continue; that switching sync off or Clear All removes those entries; that the preference record is written only when you tap Sync Now; and that neither the switch nor Clear All deletes that record. The earlier page said only seven settings sync.
- **Handoff.** It says Handoff tells your other devices which tab and topic you are on, whether or not iCloud Sync is on.
