...
| Does anything sync through iCloud? | Yes, a little, on iPhone and iPad only: a small iCloud key-value store holds which tab and which library topic you were on, the topic the Home Screen widget suggests, and your appearance choice. None of it is an amount or anything you typed. See [iCloud and syncing](#icloud-and-syncing). |
...
Neither notification is sent through a server. **Delete all data** does not turn the reminder off; use the switch.
...
**What Delete all data does not remove.** **Delete all data in this app** does not clear these four iCloud values, and deleting the app does not remove them from iCloud either. They stay in your iCloud key-value store, and on your other devices that use the same Apple Account, until the app overwrites them: the tab and topic when you move to another tab or topic, the widget's topic when its suggestion changes, and your appearance when you change it. A queued topic is cleared as soon as the app opens it. Apple stores these values under your Apple Account; Prameya cannot see or delete them.
...
- **Delete everything you have entered:** **More ▸ Data ▸ "Delete all data in this app"**. After a confirmation prompt, this permanently deletes, on this device, every habit log, envelope / budget line, statement record and its photo, saved scenario, and your profile (goals and income figure), plus your age-range result, and leftover model files and settings from older installs if any remain. If a later file removal fails after the database is emptied, the app says so rather than claiming a complete delete. It cannot be undone. It does **not** clear the four iCloud key-value values described under [iCloud and syncing](#icloud-and-syncing), turn off the weekly reminder, reset your appearance or Ask settings, or touch the Watch complication label; and it cannot reach copies already in your device backups or in files you exported.
- **Delete everything on a device, including preferences:** delete the app. That removes the app's local database and its preferences from that device. It does not remove the iCloud key-value values described under [iCloud and syncing](#icloud-and-syncing).
