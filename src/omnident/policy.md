...
- **They are not synced to iCloud by OmniDent.** The local photo database is configured with cloud sync switched off entirely, and no photo, thumbnail or file path is ever put in the iCloud key-value store or in Handoff.
...
- Everything: **Settings → Privacy & Security → Delete All Scans & Data**. This is immediate and permanent. Section 10 lists everything it removes, and the few things it leaves.
- If you turned on auto-save to Photos, or saved a photo there, copies in your Photos library are yours to delete in the Photos app. OmniDent cannot reach into your Photos library to remove them, and does not try to.
...
- The **Ask** tab appears where Apple Intelligence is turned on and ready and on-device answers are on. It is hidden in a child session.
- Your question goes to Apple Intelligence on your device. Apple Intelligence may look things up in OmniDent's built-in reference library, which is also on your device. If Apple Intelligence cannot answer that question, Ask tries a text model you downloaded, if there is one.
...
- **We do not share your data with dentists, insurers, employers or partners.** The app no longer lists partner offers. If you claimed one in an earlier version, the claim is a note on your device and nothing was transmitted.
...
- **We do not contribute your data to any shared or "collective" dataset.**
...
- On iPhone, iPad and Apple Vision Pro, the files OmniDent creates — photographs, documents and its database — default to Data Protection at the Complete level: encrypted at rest and inaccessible while the device is locked. On Mac, app data sits in the app's sandbox container and is protected by the Mac's own disk encryption if you have turned it on.
- Your Apple sign-in identifier, name and email are stored in the Keychain, on this device only.
- The household transfer file is encrypted with a passphrase you choose (at least eight characters).
...
- **Legal bases.** Where processing occurs, we rely on your **consent** (Article 6(1)(a)) for optional features such as camera access, AI features, iCloud Sync and Apple Health, and on **contract** (Article 6(1)(b)) for the core operation of the app you installed.
...
- **International transfers.** We transfer none of your content, because we receive none. The only cross-border flows are your own connection to Hugging Face for a model file, and what OmniDent puts in your own iCloud account (section 6), under Apple's arrangements.
...
- It says that, while iCloud Sync is on (the default), OmniDent writes a short continue note to your iCloud key-value store — the tab, the last education topic you opened, a photo-record identifier, and the name of the active household person — and that Apple TV and Apple Watch show that name. The previous version said nothing but preferences went to iCloud.
- It says Handoff carries the same note to your nearby devices whether or not iCloud Sync is on.
- It describes Ask (Apple Intelligence on your device first, a downloaded model second, answers checked against a word list), and the "What this photo looks like" reading, which shows the models' output unfiltered and can name a condition. The previous version said photo output never named a condition and that chat was unfiltered.
- It names the three models offered now (Qwen2.5 0.5B, Gemma 4 E2B, SmolVLM) and says Llama 3.2 1B, Gemma 3 4B and PaliGemma 3B were removed and are deleted from your device; that downloads wait for Wi-Fi unless you allow cellular; and that a question from Apple Watch can start a download on your iPhone.
- It adds the Apple Watch, Apple TV, Mac and Apple Vision Pro apps, care reminders, and the Lock Screen care timer.
- It says a brush confirmed from Apple Watch or a reminder writes to Apple Health even when OmniDent's Apple Health switch is off; that the 30-day programme and the Watch can raise iOS's Apple Health permission request while the switch is off; exactly which entries are written; and that the app does not currently read any Apple Health data.
- It says the date of each visit you log is added to your device's Spotlight search, and that Delete All does not remove those entries.
...
- **The 20 September 2026 revision** matched the shipping household: people and mouths on this device, a child session that cannot see another mouth, Face ID or the device passcode to leave a child session, auto-save to Photos forced off while a child is active, visits and documents on Record, StoreKit on the network list, and the Home Screen widget's care-status line. **The 24 August 2026 revision** updated the export section so the export is a `.zip` that carries the photographs themselves, and so the deletion table names the per-record controls.
- **The 21 August 2026 revision** matched the shipping code on auto-save captures to Photos (**off** by default), withdrew the blanket "does not diagnose" wording in favour of describing the photo note and the chat, and corrected the Studio Regenerative Projection percentages.
- **The 8 August 2026 revision was a correction pass.** We re-read the shipping source code and rewrote every statement that did not match it. In particular: the previous version said that deleting your account revokes your Sign in with Apple token through Apple's REST API. It does not, and it never did — the app has no server and mints no token, so there is nothing to revoke; section 5 now describes what deletion actually does.
...
- Previous versions are available on request from admin@prameya.legal, and in the public repository that publishes these pages: <https://github.com/prameyallc/privacy>.
